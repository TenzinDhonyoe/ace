...
> **Status:** pre-0.3. The packages `ace-study-components` and `ace-study-template`
> are not yet published to npm, so `bunx` / `unpkg` commands in this README don't
> work yet. **Clone the repo** and use the local workflow below. See
> [PUBLISHING.md](PUBLISHING.md) for the publish plan.
...
**Edit without regenerating:** hand-edit `output/site.config.json`, then re-render with the monorepo CLI:
```bash
bun packages/ace-template/bin/cli.js output/site.config.json -o output/
```
Once the packages are on npm, `bunx ace-study-template output/site.config.json -o output/` will do the same thing from any directory.
...
> Until the packages hit npm, use the local source (`packages/ace-components/`) or
> copy the built bundle from `output/` after running `/ace-review` once.
