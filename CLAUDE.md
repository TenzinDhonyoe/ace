...
bun packages/ace-template/bin/cli.js output/site.config.json -o output/   # manual render after editing config
```
...
- **"edit a widget"** → they should hand-edit `output/site.config.json` and re-run `bun packages/ace-template/bin/cli.js output/site.config.json -o output/`. Don't regenerate from PDFs.
